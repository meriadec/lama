...
import co.ledger.lama.bitcoin.worker.models.FindKeychainAddressesFailed
...
class WorkerWithFaultyKeychainIT extends AnyFlatSpecLike with Matchers {
            ...
            .map { reportableEvent =>
              it should "report a failed synchronization because of lack of addresses" in {
                for {
                  ...
                  re.payload.data shouldBe FindKeychainAddressesFailed(keychainId).errorMessage
                }
              }
            }
        }
